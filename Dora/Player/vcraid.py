...
from Dora.queues import QUEUE, add_to_queue, get_queue, clear_queue

from Dora.main import call_py, bot as Dora, Test, call_py2, call_py3, call_py4, call_py5
...
@Dora.on_message(filters.user(SUDO_USERS) & filters.command(["vcraid"], prefixes=HNDLR))
async def vcraid(_, e: Message):
    ...
    if inp:
        Dora = await e.reply_text("**Starting Raid**")
        link = f"https://Dora-robot.github.io/{aud[1:]}"
        dl = aud
        songname = aud[18:]
        if chat_id in QUEUE:
...
@Dora.on_message(filters.user(SUDO_USERS) & filters.command(["vraid"], prefixes=HNDLR))
...
@Dora.on_message(filters.user(SUDO_USERS) & filters.command(["raidend"], prefixes=HNDLR))
...
@Dora.on_message(filters.user(SUDO_USERS) & filters.command(["raidpause"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
...
@Dora.on_message(filters.user(SUDO_USERS) & filters.command(["raidresume"], prefixes=HNDLR))
async def ping(_, e: Message):
    gid = e.chat.id
    uid = e.from_user.id
    if gid == uid:
